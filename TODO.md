...
- [] add convertion from rotation matrix to quaternions *(with $q_w \geq 0$)*
- [] add normalisation to the bricks positions
    - first put all sets around $(0, 0, 0)$
...
- create `metadata.json` to store:
    - the scale so the model is able to multiply the normalised generated positions to their actual full size position
